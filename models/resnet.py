...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, num_filters, block_name='BasicBlock', num_classes=10):
        ...
        # self.relu = nn.ReLU(inplace=True)
        self.relu = nn.ReLU(inplace=False) # for afd
...
#             SepConv(
#                 channel_in=num_filters[1] * block.expansion,
#                 channel_out=num_filters[1]*2 * block.expansion
#             ),
#             SepConv(
#                 channel_in=num_filters[1]*2 * block.expansion,
#                 channel_out=num_filters[1]*4 * block.expansion
#             ),
#             nn.AvgPool2d(8, 8)
#         )
...
#             SepConv(
#                 channel_in=num_filters[2] * block.expansion,
#                 channel_out=num_filters[2]*2 * block.expansion,
#             ),
#             nn.AvgPool2d(8, 8)
#         )
...
#         out1_feature = self.auxiliary1(f1).view(x.size(0), -1)
#         out2_feature = self.auxiliary2(f2).view(x.size(0), -1)
#         out3_feature = self.auxiliary3(f3).view(x.size(0), -1)
...
#         if is_feat:
#             if preact:
#                 return [f0, f1_pre, f2_pre, f3_pre, f4], [out1, out2, out3]
#             else:
#                 return [out1_feature, out2_feature, out3_feature], [out1, out2, out3]
#         else:
